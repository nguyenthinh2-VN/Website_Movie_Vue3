...
export const useCategoryMovieStore = defineStore('categoryMovie', {
  ...
  getters: {
    ...
    categoryInfo: (state) => state.currentCategory
  },

  actions: {
    async fetchMoviesByCategory(categorySlug, page = 1, silent = false) {
      if (!silent) {
        this.loading = true
        this.error = null
      }
      ...
      try {
        const response = await fetch(`https://phimapi.com/v1/api/the-loai/${categorySlug}?page=${page}`)
        ...
        if (data.status) {
          ...
          if (data.data && data.data.items) {
            ...
            if (data.data.params && data.data.params.pagination) {
              ...
              this.pagination = {
                ...
                currentPage: page,
                totalPages: Math.ceil(this.movies.length / 10)
              }
            }
            
            // Lưu thông tin thể loại từ titlePage
            if (data.data.titlePage) {
              this.currentCategory = {
                name: data.data.titlePage,
                slug: categorySlug
              }
            }
          ...
          throw new Error(data.msg || 'Failed to fetch category movies')
        }
